  ...
  return (
    <div className="page">
      ...
      {!data ? <div className="skeleton" /> : <>
        ...
        <div className="grid grid-2 report-summary"><section className="card card-body"><h2>프린터별 작업</h2>{data.printer_stats.length ? data.printer_stats.map((item) => <div className="bar-row" key={item.name}><div><strong>{item.name}</strong><span>{item.completed} 완료 · {item.failed} 실패</span></div><div className="bar"><span style={{ width: `${data.stats.total ? item.total / data.stats.total * 100 : 0}%` }} /></div><strong>{item.total}</strong></div>) : <p className="muted">이번 달 기록이 없습니다.</p>}</section><section className="card card-body"><h2>완료 작업 상위 이용자</h2>{data.top_users.length ? <ol className="top-users">{data.top_users.map((item) => <li key={item.email}><div><strong>{item.name}</strong><span>{item.email}</span></div><b>{item.count}</b></li>)}</ol> : <p className="muted">완료된 작업이 없습니다.</p>}</section></div>
        <section className="admin-section"><div className="section-heading"><div><h2>{year}년 {month}월 전체 기록</h2><p>신청 시각 기준으로 정렬됩니다.</p></div></div>{data.jobs.length ? <div className="card table-wrap"><table className="table"><thead><tr><th>신청</th><th>학생</th><th>파일</th><th>프린터</th><th>상태</th><th>완료</th></tr></thead><tbody>{data.jobs.map((job) => <tr key={job.id}><td className="muted">{formatDate(job.created_at)}</td><td>{job.owner?.name}</td><td><strong className="truncate report-filename">{job.filename}</strong></td><td>{job.printer?.name}</td><td><StatusBadge status={job.status} /></td><td className="muted">{formatDate(job.completed_at)}</td></tr>)}</tbody></table></div> : <div className="admin-empty">이 기간의 작업이 없습니다.</div>}</section>
      </>}
    </div>
  );
}
