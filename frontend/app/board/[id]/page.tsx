  ...
  return (
    <div className="page page-narrow">
      ...
      <article className="card post-card">
        <header className="post-header"><h1>{data.post.title}</h1><div className="post-meta"><strong>{data.post.author?.name.replace(/^\d+/, "")}</strong><time>{formatDate(data.post.created_at, true)}</time><span>댓글 {data.comments.length}개</span>{canDeletePost ? <button className="button button-danger button-small" onClick={deletePost}><Trash2 size={13} /> 삭제</button> : null}</div></header>
      ...
      <section className="comments" aria-labelledby="comments-title">
        ...
        <div className="comment-list">{data.comments.map((item) => {
          ...
          return <article className="comment" style={{ "--depth": Math.min(item.depth, 4) } as React.CSSProperties} key={item.id}><header><strong>{item.author?.name.replace(/^\d+/, "")}</strong><time>{formatDate(item.created_at)}</time><button className="button button-ghost button-small" onClick={() => { setReplyTo(replyTo === item.id ? null : item.id); setReplyBody(""); }}><Reply size={13} /> 답글</button>{canDelete ? <button className="button button-ghost button-small danger-text" onClick={() => deleteComment(item.id)}>삭제</button> : null}</header><p><LinkedText>{item.body}</LinkedText></p>{replyTo === item.id ? <div className="reply-form"><textarea className="textarea" value={replyBody} onChange={(event) => setReplyBody(event.target.value)} placeholder={`${item.author?.name.replace(/^\d+/, "")}님에게 답글`} autoFocus /><div className="form-actions"><button className="button button-primary button-small" disabled={busy} onClick={() => comment(replyBody, item.id)}>답글 등록</button><button className="button button-secondary button-small" onClick={() => setReplyTo(null)}>취소</button></div></div> : null}</article>;
        })}</div>
