...
# Docker Compose는 env_file로 설정을 주입하며, 로컬 실행을 위해 .env도 로드합니다.
load_dotenv()

APP_DIR = Path(__file__).resolve().parent
LOCAL_DATA_DIR = APP_DIR.parent / "data"
...
class Settings:
    ...
    DATABASE_URL: str = os.getenv(
        "DATABASE_URL",
        f"sqlite+aiosqlite:///{(LOCAL_DATA_DIR / 'db.sqlite').as_posix()}",
    )

    # 업로드 폴더
    UPLOAD_DIR: str = os.getenv("UPLOAD_DIR", str(LOCAL_DATA_DIR / "uploads"))

    # 게시판 첨부 파일 폴더
    BOARD_UPLOAD_DIR: str = os.getenv(
        "BOARD_UPLOAD_DIR", str(LOCAL_DATA_DIR / "board")
    )
